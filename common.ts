...
import { readableStreamToString, ConnexPayError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new ConnexPayError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new ConnexPayError(e, e.response?.data)
                }
                if (innerError instanceof ConnexPayError) {
                    // Got "ConnexPayError" from the above try block
                    throw innerError;
                }
